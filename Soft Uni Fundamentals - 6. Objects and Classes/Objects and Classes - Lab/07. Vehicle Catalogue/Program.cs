...
class VehicleCatalogue
{
    static void Main(string[] args)
    {
        ...
        while ((input = Console.ReadLine()) != "end")
        {
            ...
            int powerOrWeight = int.Parse(parts[3]);

            if (type == "Car")
            {
                catalog.Cars.Add(new Car { Brand = brand, Model = model, HorsePower = powerOrWeight });
            }
            else if (type == "Truck")
            {
                catalog.Trucks.Add(new Truck { Brand = brand, Model = model, Weight = powerOrWeight });
            }
            else
            {
